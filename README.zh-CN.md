...
| 数据架构 · 横向流向板式 | 纯净 PNG 导出(独立产物,含当前配色) |
| ![数据架构横向流向板式](docs/images/flow-preview.png) | ![纯净 PNG 导出](docs/images/export-data.png) |
...
- **自动排版**:同一份内容在 2 种板式间切换,数据不丢失——多层横向、横向流向(旧卡片/中央数据打开自动转多层横向)
- **规范固化**:虚线面板、圆角 2px、字号层级;6 套预置配色(含分层展示语义 token)+ 自定义六档色阶,随图保存(含右侧通栏竖条)
...
- **12 个内置模板**:通用分层 / 总体架构 / 应用架构 / 数据架构(数仓分层)/ 技术架构 / 中台架构 / 云平台架构 / 微服务架构 / 安全架构(等保 2.0) + 3 个横向流向模板:数据流转 / 服务调用链路 / 事件流
- **层间连线**:可选的层间连线(仅 layered 布局),支持箭头、标签、线型;视图菜单可开关
- **模块角标**:条目中 `[调用:xx]` / `[依赖:xx]` 格式自动渲染为角标,全布局生效
- **导出 draw.io**:逃生通道,导出 `.drawio` XML 文件,可在 draw.io 中补充自由曲线与复杂排版
...
| 中台架构 | 多层横向 | 用户接入 → 业务应用 → 共享服务中心(业务中台) → 数据中台 → 技术底座 |
| 云平台架构 | 多层横向 | 接入访问 → 云管理平台 → SaaS 应用 → PaaS 平台 → IaaS 资源 → 基础设施 |
| 微服务架构 | 多层横向 | 接入层(网关/负载) → 服务层(业务服务+角标) → 公共组件层 → 数据层(含演示连线) |
| 安全架构(等保 2.0) | 多层横向 | 安全物理环境 → 安全通信网络 → 安全区域边界 → 安全计算环境 → 安全管理中心 |
| 数据流转·横向流向 | 横向流向 | 数据源 → 数据集成 → ODS 贴源 → DWD 明细 → DWS 汇总 → ADS/数据服务(带标签箭头) |
| 服务调用链路·横向流向 | 横向流向 | 客户端 → 接入层 → 服务层 → 数据层 → 监控审计(调用链流向) |
| 事件流·横向流向 | 横向流向 | 事件源 → 消息平台 → 实时计算 → 落地存储(事件驱动流向) |
...
| **draw.io** | 逃生通道:导出 `.drawio` XML,可到 draw.io 里补自由曲线 |
...
├─ editor-render.js     # 渲染引擎:多层横向 / 横向流向
